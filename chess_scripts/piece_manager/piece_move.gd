...
static func get_moves(current_coords:Vector2i) -> Array:
	...
	match abs(Scripts.PIECE_MANAGER.get_piece_data(current_coords,Scripts.PIECE_CONSTS.PIECE_LIST.TYPE)):
		...
		Scripts.PIECE_CONSTS.TYPE_LIST.QUEEN:
			_moves = get_rook_moves(current_coords)
			_moves.append_array(get_bishop_moves(current_coords))
		Scripts.PIECE_CONSTS.TYPE_LIST.KING:
			_moves = get_king_moves(current_coords)
	print("piece_move/get_moves- _moves: ",_moves)
	return _moves

static func is_move_possible(asked_coords:Vector2i) -> bool:
	if is_valid_position(asked_coords):
		if is_empty(asked_coords):
			return true
		if is_enemy(asked_coords):
			return true
	return false
...
static func is_enemy(asked_coords:Vector2i) -> bool: # TODO: Finish this. NOT WORKING YET !!!!!!!! ONLY REFRENCE !!!!
...
static func get_knight_moves(current_coords:Vector2i) -> Array:
	_moves = []
